...
int read_confdata(char *dataformat_dir,char ***confa,char ***confb)
{
    int i,len;
    char* pContent;

    FILE* fp = fopen(dataformat_dir, "rb+");
    if(fp==NULL)
    {
...
char *data_format(char *dataformat_dir,char *buf_data,char *buf_tmp)
{
    int i;
    char sep[2];
    char **confa = NULL;
    char **confb = NULL;
    int sum = read_confdata(dataformat_dir,&confa,&confb);
    DEBUG_printf("sum = %d\n",sum);
