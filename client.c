...
int sock_connect_server()
{
    ...
    #endif // OPENWRT
    #ifdef BB_BLACK
    read_conf_by_type(bb_black_conf_dir,"ip_server",ip_str);
    #endif // BB_BLACK
...
void *read_switch(void)
{
    while(p_n_over)
    {
        ...
        #ifdef BB_BLACK
        char key_switch[15];
        if(strcmp("off",read_bb_black_switch(bb_black_conf_dir,BB_BLACK_CONF_SWITCH,key_switch))==0)
        {
            SW=1;
        }
        printf("key_switch = %s\n",key_switch);
    ...
    while(p_n_over)
    {
        if(0!=NUL_Queue(&Q_respond))
        {
            if(DeQueue(&Q_respond,&deal_package_resp))
            {
                ...
                rev_respond();
                DEBUG_printf("response node:%s,%s.\n",deal_package_resp.pkg_cmd,deal_package_resp.pkg_nonce);
                Position curnode = Find(&deal_package_resp,L_cmd);
                if(curnode == NULL)
                {
                    DEBUG_printf("no cmd for responding!\n");
                    exit(1);
                }
                if(strcmp(curnode->item->pkg_cmd,"01") == 0)
                {
                    #ifdef BB_BLACK
                    write_by_type(bb_black_conf_dir,deal_package_resp.pkg_cmd);
                    #endif // BB_BLACK
                    #ifdef OPENWRT
                    write_openwrt_auth(deal_package_resp.pkg_cmd);
                    #endif // OPENWRT
                }
                DEBUG_printf("Delete node:%s,%s.\n",deal_package_resp.pkg_cmd,deal_package_resp.pkg_nonce);
...
void set_dconfig_flag()
{
    if(MINERTYPE == 5)
    {
        write_bb_black_respond(bb_black_conf_dir,"fx");
    }
    else if(MINERTYPE == 3)
    {
        write_openwrt_auth("fx");
    }
}
void Initsem()
{
...
bool goon()//If another process is running,then stop!(singleton)
{
    extern int errno;
    int pid;
