...
int write_config_s3(PKG_Format pkg_struct)
{
   ...
   new_pools_data=creat_openwrt_newpools(rev_pools(pkg_struct.pkg_body,&changed_pools),save_openwrt_webconf(&gets3_cgminer_conf));
   write_openwrt_conf(new_pools_data);
   //restart_cgminer();
   return 1;
}
...
char restart_cgminer()  //restart  cgminer
{
    ...
    #ifdef BB_BLACK
    char port_str[10];
    cg_port=atoi(read_bb_black_switch(bb_black_conf_dir,"port_local",port_str));
    #endif // BB_BLACK
